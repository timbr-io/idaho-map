...
import CanvasTileLayer from './canvas_tile_layer';
...
export default class Footprints extends Component {
  ...
  _renderBox( ctx, bbox, project, width, height, stroke = '#ffffff', strokeWidth=1) {
  ...
  _renderGeom( loc, ctx, zoom, project, width, height ) {
    ...
    if ( Math.floor( zoom ) < 5 ) {
      ...
      tiles.forEach( tile => {
        ...
        const _drawn = this._renderBox( ctx, bbox, project, width, height, '#999' );
        if ( _drawn ) {
